...
function addFonts() {
  ...
  font_css.setAttribute(
    "href",
    "//cdn.bootcss.com/material-design-iconic-font/2.2.0/css/material-design-iconic-font.min.css"
  );
...
function popupThumb() {
  ...
  var thumbLeft = Math.max(
    0,
    cover_mini_pos.left + cover_mini_width / 2 - 177 / 2 - 2.5
  );
  var thumbLeftRatio = Math.max(
    0,
    (cover_mini_pos.left + cover_mini_width / 2) / view_width
  );

  var cover_thumb = cover_mini
    .attr("src")
    .replace("34x34", "177x177")
    .replace("34y34", "177y177");
  var thumb_options = {
    closeClick: true,
    overlayShow: false,
    helpers: {
...
function saveToFile() {
  ...
  if (album_title != "")
    fileNameToSaveAs =
      "intro_" + album_title.replace(/[\/\\|"<>\^]/gu, "-") + ".md";
...
function getMVid(musicid) {
  ...
  url =
    protocol +
    "//music.163.com/api/song/detail/?id=" +
    musicid +
    "&ids=[" +
    musicid +
    "]";
...
function ConvertToMarkdown() {
  var loc = window.location.href
    .replace(/http(s){0,1}:\/\/music\.163\.com\/(#\/){0,1}/gim, "")
    .toLowerCase();
  ...
  var title = content
    .find("div.m-info div.tit h2")
    .text()
    .trim()
    .replace("|", "&#x7c;");
  var sub_title = content
    .find("div.m-info div.subtit")
    .text()
    .trim()
    .replace("|", "&#x7c;");
  ...
  if (intrs.length > 0) {
    ...
    artist_name = artist.title.trim().replace("|", "&#x7c;");
    artist_href = $(artist)
      .find("a")[0]
      .href.trim();
  ...
  pub_date = "";
  if (intrs.length > 1) pub_date = intrs[1].lastChild.textContent.trim();
  pub_corp = "";
  if (intrs.length > 2) pub_corp = intrs[2].lastChild.textContent.trim();
  ...
  songs.each(function(idx) {
    ...
    if (trk_name.length > 0) trk_name = trk_name[0].textContent.trim();
    else trk_name = songinfo[1].textContent.trim();
    var trk_comment = $(songinfo[1]).find("span.s-fc8");
    if (trk_comment.length > 0)
      trk_comment =
        " " + trk_comment[0].textContent.trim().replace("|", "&#x7c;");
    else trk_comment = "";
    ...
    var trk_time = $(songinfo[2])
      .find("span.u-dur")[0]
      .textContent.trim();
    //console.log(trk_time);
    var trk_artists = $(songinfo[3])
      .find("span")[0]
      .textContent.trim()
      .replace("|", "&#x7c;");
    ...
    $(trk_artistlist).each(function(id) {
      ...
      if (art.nodeName == "A") {
        trk_artistall +=
          "[" +
          art.textContent.trim().replace("|", "&#x7c;") +
          "](" +
          art.href +
          "){.pop} ";
      } else {
        //else if(art.nodeName=='SPAN')
        trk_artistall += art.textContent.trim().replace("|", "&#x7c;") + " ";
      }
    });
  ...
  md += '<div class="video">\n';
  md +=
    '  <video id="mv00" class="video" controls src="" preload="metadata">\n';
  md += '    <track label="English" kind="subtitles" srclang="en" />\n';
  md += '    <track label="Japanese" kind="subtitles" srclang="jp" />\n';
  md +=
    '    <track label="Chinese" kind="subtitles" srclang="cn" src="" default />\n';
  md += "  </video>\n";
  md += "</div>\n\n";
...
function addToMarkdown() {
  var loc = window.location.href.replace(
    /http(s){0,1}:\/\/music\.163\.com\/(#\/){0,1}/gim,
    ""
  );
  ...
  var contentOp = $(".nav")[0];
  $(
    '<li><a id="btnMarkdown" hidefocus="true" href="javascript:;" title="Convert to Markdown"><em>Markdown</em></a></li>'
  ).appendTo(contentOp);
  ...
  var saveButtonStyle =
    ...
    "  border-color: #8c8c8c;\n" +
    "}\n";
  $('<style type="text/css">\n' + saveButtonStyle + "</style>").appendTo(
    $("head")
  );
...
function updateAlbumIcon() {
  $("iframe#g_iframe.g-iframe").load(function() {
    ...
    headset.attr(
      "style",
      "float:left;width:14px;height:11px;margin:9px 5px 9px 10px;"
    );
    ...
    content
      .find(".info .sign")
      .addClass("zmdi zmdi-assignment-o mdc-text-light-blue");
    content
      .find(".info .u-btn2.u-btn2-dis")
      .addClass("zmdi zmdi-assignment-check mdc-text-green");
